...
const uint TILE_HEIGHT = 12;

#include "inline/stdTileSet.inl"
...
sf::Sprite* StdTilePool::getStdTile( const wstring& name )
{
...
bool StdTilePool::isStdTileName( const wstring& name )
{
 ...
 *
 */
void StdTilePool::loadTextureFromSource( const sf::Uint8* src, uint length)
{
	...
	const uint tilesPerRow = imgSize / TILE_WIDTH;
	const uint tileRows = imgSize / TILE_HEIGHT;

	uint tilesExtracted = 0;

	for(uint row = 0; row < tileRows; ++row)
	{
		for(uint col = 0; col < tilesPerRow; ++col)
		{
			...
			const uint ybegin = row * TILE_HEIGHT;
			const uint yend = ybegin + TILE_HEIGHT;

			for(uint y = ybegin; y < yend; ++y)
			{
				byte rowContent = *src; ++src;
				const uint xbegin = col * TILE_WIDTH;
				
				for(uint x = xbegin; rowContent; ++x)
				{
					if(rowContent & 0x80)
					{
...
void StdTilePool::addStdTile( uint col, uint row, const wstring& name )
{
	mTilesAssociation.insert(
		std::make_pair(
			name,
			make_shared<sf::Sprite>(mTileImage, 
